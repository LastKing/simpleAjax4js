/**
 * 权限验证
 * Created by toonew on 15-12-22.
 */
import request from 'supertest';
import path from 'path';
import should from 'should';

import serverInit from '../assets/lib/server-init';
...
before(async(done)=> {
  ...
  try {
    //获得普通用户权限
    request(app)
        .get('/simpleajax/account/login.ac?args=["user1","user1"]')
        .expect(200)
    ...
    //获得 管理员用户权限
    request(app)
        .get('/simpleajax/account/login.ac?args=["admin","admin"]')
        .expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
        .end((err, res)=> {
          if (err)done(err);
...
describe('权限测试', ()=> {
  ...
  it('不带权限调用getPublicInfo', (done)=> {
    request(app)
        .get('/simpleajax/permission/getPublicInfo.ac')
        .expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
        .end((err, res)=> {
          if (err)done(err);
  ...
  it('不带权限调用getInternalInfo', (done)=> {
    request(app)
        .get('/simpleajax/permission/getInternalInfo.ac')
        .expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
        .end((err, res)=> {
          if (err)done(err);
  ...
  it('带普通用户权限调用getInternalInfo', (done)=> {
    //登陆成功,开始发送带权限的请求
    var req = request(app).get('/simpleajax/permission/getInternalInfo.ac');
    //回复登陆会话的cookie
  ...
  it('带普通用户权限调用getUser,username不是自己', (done)=> {
    //登陆成功,开始发送带权限的请求
    var req = request(app).get('/simpleajax/permission/getUser.ac?args=["user2"]');
    //回复登陆会话的cookie
    req.cookies = userSession;
    req.expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
  ...
  it('带普通用户权限调用getUser,username是自己', (done)=> {
    //登陆成功,开始发送带权限的请求
    var req = request(app).get('/simpleajax/permission/getUser.ac?args=["user1"]');
    //回复登陆会话的cookie
    req.cookies = userSession;
    req.expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
  ...
  it('带普通用户权限调用getAllUser', (done)=> {
    //登陆成功,开始发送带权限的请求
    var req = request(app).get('/simpleajax/permission/getAllUser.ac');
    //回复登陆会话的cookie
    req.cookies = userSession;
    req.expect(200)
        .expect('Content-Type', 'application/json; charset=utf-8')
